...
class load_table extends dts_table
{
	...
	function get_search_results()
	{
		...
		return $t->fetch(App::$temp.'load_search_result.tpl');
	}
	...
	function get_search_edit()
	{
		...
		return $t->fetch(App::$temp.'load_search_form.tpl');
	}
	...
	function fetch_warehouses_old2()
	{
		$sql = "SELECT	*
			, CASE DAYNAME(activity_date)
				WHEN 'Sunday' THEN DATE_FORMAT(sun_close_time, '$this->time_format')
				WHEN 'Monday' THEN DATE_FORMAT(mon_close_time, '$this->time_format')
				WHEN 'Tuesday' THEN DATE_FORMAT(tues_close_time, '$this->time_format')
				WHEN 'Wednesday' THEN DATE_FORMAT(wed_close_time, '$this->time_format')
	...
	function get_warehouses()
	{
		...
		return $t->fetch(App::$temp.'load_warehouse_mod.tpl');
	}
	
	function get_warehouses_old()
	{
		//Needs work
		require_once"includes/portal.php";
	...
	function get_customer_search_result_module()
	{
		...
		return $t->fetch(App::$temp.'/cust_search_result.tpl');
	}
	function get_customer_search_result_module_old()
	{
		require_once("includes/portal.php");
		$c ='<!-- customer_search_result_module -->';
	...
	function get_money_module()
	{
		...
		$t->assign('wcp', $wcp);
		$t->assign('dtsp', $dtsp);
		return $t->fetch(App::$temp.'money_load.tpl');
	}
	...
	,2) profit
		...
		/*echo "loss:$r[loss]<br>";
		echo "gross:$r[gross]<br>";
		echo "profit:$r[profit]<br>";
		echo $r['wc_percent'];*/
		return $r['profit'];
	}
		...
		function get_loads($date, $type)
	{
		...
		return $t->fetch(App::$temp.'load_list.tpl');
	}
	...
	function get_rate_conf()
	{
		...
		$t->assign('pick', DB::to_array($this->get_pickups($_REQUEST["load_id"])));
		$t->assign('drop', DB::to_array($this->get_drops($_REQUEST["load_id"])));
		
		$line_haul_total = $r['line_haul_amount'] * $r['carrier_line_haul'];
		$t->assign('line_haul_total', $line_haul_total);
		
		$detention_total = $r['detention_amount']*$r['carrier_detention'];
		$t->assign('detention_total', $detention_total);
		
		$tonu_total = $r['tonu_amount']*$r['carrier_tonu'];
		$t->assign('tonu_total', $tonu_total);
		
		$unload_load_total = $r['unload_load_amount']*$r['carrier_unload_load'];
		$t->assign('unload_load_total', $unload_load_total);
		
		$fuel_total = $r['fuel_amount']*$r['carrier_fuel'];
		$t->assign('fuel_total', $fuel_total);
		
		$other_total = $r['other_amount']*$r['carrier_other'];
		$t->assign('other_total', $other_total);
		
		$t->assign('grand_total', $line_haul_total + $detention_total + $tonu_total + $unload_load_total + $fuel_total + $other_total);
		$t->assign('load', $r);
		return $t->fetch(App::$temp.'rate_conf.tpl');
	}
	
	function get_rate_conf_old()
	{
		$sql = "SELECT	l.load_id,
						l.class,
						l.commodity,
					...
					<td>
						<table border=0 width=100%>
							<tr>
								...
								<div style=\"margin-right:auto;margin-left:auto;font-size:16pt;padding:3pt;width:4em;\" class='bold heavy_frame'>$_REQUEST[load_id]</div>
								</td>
								<td width=30% style='vertical-align:middle'>";
		if($r['ltl_number'])
		{
		$header .= "
									<div style=\"text-align:center;margin-right:auto;margin-left:auto;padding:6pt;\" class='bold heavy_frame'>$r[pro_number]</div>
									<div style=\"text-align:center;padding:6pt;font-size:.8em\" class='bold heavy_frame'>$r[ltl_number]</div>";
		}
		$header .= "
								</td>
							</tr>
				...
				<tr>
					<td style=\"text-align:center\">
						...
						For distpatch at 847-981-1400 and ask for load # $r[load_id]<br>Domestic Transport Solutions has 24 hour dispatch. Call anytime.
					</td>
				</tr>
			</table>
			<hr />";
		...
		while($pick = db_fetch_assoc($picks))
		{
			$origin .= "$sep
				<tr>
					<td>Shipper:</td><td class='bold'>$pick[origin_name]</td>
					<td width='$gutter'></td>
					<td>Pick&nbsp;Up&nbsp;Date:</td>
					<td class='bold'>$pick[pickup_date]</td>
				</tr>
				<tr>
					<td rowspan=2>Address:</td>
					<td rowspan=2 class='bold'>$pick[origin_address]<br>$pick[origin_city] $pick[origin_state] $pick[origin_zip]</td>
					<td width='$gutter'></td>
					<td>Pick Up Time:</td><td class='bold'>".nbsp($pick['pickup_time'])."</td>
				</tr>
				<tr>
					<td width='$gutter'></td>
					<td>Pick #:</td><td class='bold'>$pick[pick_num]</td>
				</tr>
				<tr>
					<td>Phone:</td><td class='bold'>$pick[origin_phone]</td>
					<td width='$gutter'></td>
					<td>Notes:</td><td class=\"bold\">$pick[origin_notes]</td>
				</tr>";
				$sep = "
				<tr>
					<td colspan=\"5\"><hr /></td>
		...
		//$sep = '';
		while($drop = db_fetch_assoc($drops))
		{
			
			$dest .= "$sep
				<tr>
					<td>CONS:</td><td class='bold'>$drop[dest_name]</td>
					<td width='$gutter'></td>
					<td>Delivery Date:</td><td class='bold'>$drop[delivery_date]</td>
				</tr>
				<tr>
					<td rowspan=2>Address:</td><td rowspan=2 class='bold'>$drop[dest_address]<br>$drop[dest_city] 	$drop[dest_state] $drop[dest_zip]</td>
					<td width='$gutter'></td>
					<td>Delivery&nbsp;Time:</td><td class='bold'>".nbsp($drop['delivery_time'])."</td>
				</tr>
				<tr>
					<td width='$gutter'></td>
					<td>Dest #:</td><td class='bold'>$drop[dest_num]</td>
				</tr>
				<tr>
					<td>Phone:</td><td class='bold'>$drop[dest_phone]</td>
					<td width='$gutter'></td>
					<td>Notes:</td><td class='bold'>$drop[dest_notes]</td>
				</tr>";
				$sep = "
				<tr>
					<td colspan=\"5\"><hr /></td>
		...
		$body = "
			...
			<div >Driver must ask for and receive:
				<table style=\"margin-left:auto;margin-right:auto;width:80%\">
					...
					<tr>
						<td class='bold heavy_frame'>$r[commodity]</td>
						<td class='bold heavy_frame'>$r[weight]</td>
						<td class='bold heavy_frame'>$r[size]</td>
						<td class='bold heavy_frame'>$r[class]</td>
						<td class='bold heavy_frame'>$r[pallets]</td>
						<td></td>
					</tr>
				</table>
			</div>
		...
		$carrier = "
			...
			<table style=\"margin-left:auto;margin-right:auto;width:80%;\" cellspacing=\"0\">
				<tr>
					<td>Attention:</td><td class='bold'>$r[contact_name]</td>
				</tr>
				<tr>
					<td>Carrier ID:</td><td class='bold'>S$r[carrier_id]</td>
					<td>Equipment:</td><td class='bold'>$r[trailer_type]</td>
				</tr>
				<tr>
					<td>Carrier:</td><td class='bold'>$r[carrier_name]</td>
					<td>Booked With:</td><td class='bold'>$r[booked_with]</td>
				</tr>
				<tr>
					<td>Phone:</td><td class='bold'>$r[carrier_phone]</td>
					<td>Booked Sales:</td><td class='bold'>$r[booked_salesperson]</td>
				</tr>
				<tr>
					<td>Fax:</td><td class='bold'>$r[carrier_fax]</td>
					<td class='bold' style='border:solid black;border-width: 1px 1px 1px 1px;'>Driver Name:</td>
					<td class='bold' style='border:solid black;border-width: 1px 1px 1px 0px;'>$r[driver_name]</td>
				</tr>
				<tr>
					<td>Driver Cell:</td><td class='bold'>$r[cell_number]</td>
					<td class='bold' style='border:solid black;border-width: 0px 1px 1px 1px;'>Trac/Trail#:</td>
					<td class='bold' style='border:solid black;border-width: 0px 1px 1px 0px;'>$r[tractor_number]</td>
					<td class='bold' style='border:solid black;border-width: 1px 1px 1px 0px;'>$r[trailer_number]</td>
				</tr>
			</table>
		...
		$footer = "
				...
				<table width='100%' border=0>
					<tr>
						...
						<td rowspan=2 style='font-size:.9em'>Thank You,<br />$r[booked_name]<br>Domestic Transport Solutions<br>847-981-1400 phone<br>847-981-1411 fax<br>2420 E. Oakton St Unit C<br>Elk Grove Township, IL 60005</td>
					</tr>
					<tr>
						<td valign='bottom'>
							<div style='height:20px;border-bottom:1px solid black;margin:3px'></div>
