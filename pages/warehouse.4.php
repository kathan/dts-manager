...
class warehouse_table extends dts_table
{
	...
	function warehouse_table()
	{
		...
		$c->set_value_list($this->times);
		//$c->set_value_list($this->times);
		$this->tab_menu->add_tab("?page=warehouse&action=$this->search_edit", 'Search');
	...
	function get_search_results()
	{
		...
		//echo $sql;
		$t = new Template();
		$p = new Paginator($re, $_GET['start']);
		$t->assign('pag', $p->get());
		$t->assign('warehouse', $p->to_array($re));
		$t->assign('page', $_REQUEST['page']);
		//$p->set_row_action("\"row_clicked('\$id', '\$pk', '\warehouse')\";");
		$c .= $t->fetch('/home/dts2828/www/dts/templates/warehouse_list.tpl');
	...
	function edit_warehouse()
	{
		...
		return $t->fetch('/home/dts2828/www/dts/templates/warehouse_edit.tpl');
	}
	function edit_warehouse_old()
	{
		///autosave????
		$r = $this->current_row();
