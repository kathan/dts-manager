...
$sql = " SELECT	DATE_FORMAT(l.activity_date, '%m/%e') date
	...
	{
	$sql .= ", (SELECT username FROM users u, load_carrier lc WHERE lc.load_id = l.load_id AND u.user_id = lc.booked_with) carrier_rep
			, FORMAT(IF(wc_active, profit-((profit * .01) * wc_percent), profit) * .3, 2) carrier_rep_comm
			, (SELECT username FROM users u WHERE c.acct_owner = u.user_id) sales_rep
			, FORMAT(IF(wc_active, profit-((profit * .01) * wc_percent), profit) * .1, 2) sales_rep_comm
			, FORMAT(IF(wc_active, profit-((profit * .01) * wc_percent), profit) * .4, 2) total_comm";
	}
