...
class customer_table extends dts_table
{
	...
	//Added 10/15/13
		...
		return $t->fetch(App::$temp.'cust_load_list.tpl');
	}
	...
	function get_notes()
	{
		...
		return $t->fetch(App::$temp.'cust_notes.tpl');
	}
	function get_search(){
		require_once('Template.php');
		$t = new Template();
		$t->assign('users', $this->get_users());
		return $t->fetch(App::$temp.'cust_search.tpl');
	}
	
	function get_search_old(){
	...
	function show_cust_print($cust)
	{
		...
		$c .= $t->fetch(App::$temp.'cust_print.tpl');
		return $c;
	}
	...
	function show_search_results($cust)
	{
		...
		//$acct_owners = array_merge($acct_owners, );
		//var_dump($acct_owners);
		...
		$c .= $t->fetch(App::$temp.'cust_list.tpl');
		return $c;
	}
	...
	function get_edit()
	{
		...
		$t->assign('admin', logged_in_as('admin'));
		return $t->fetch(App::$temp.'customer_edit.tpl');
	}
