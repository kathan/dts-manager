...
class load_table extends dts_table
{
	...
	function get_load_edit()
	{
		...
		$c .= "<tr><td class='label_class'>Commodity:</td><td>".$this->fetch_edit('commodity', $r['commodity'])."</td>";
		$c .= "</tr></table>";
		$c .= "		</div>";
		$c .= "</fieldset>";
		//================
	...
	function get_daily_profit_new($date)
	{
			$sql = "SELECT o.*, IF(wc_active, 0, profit-(wc_percent * profit)) profit
					FROM
				(SELECT 
					wc_active,
					wc_percent,
				round(
					...
					FROM `load` l
					WHERE activity_date = '$date'
					AND load_id in (SELECT load_id FROM load_carrier)) o";
		$result = db_query($sql);
		if(db_error())
		{
			echo db_error();
