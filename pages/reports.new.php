...
$sql = " SELECT	DATE_FORMAT(l.activity_date, '%m/%e') date
	...
	$sql .= ", CASE WHEN c.acct_owner != $_REQUEST[user_id] THEN 0  ";
	isset($_GET['wcp']) ? $sql .= "	ELSE ((profit * .01) * wc_percent) END wcp " : $sql .= ' ELSE CASE WHEN wc_active=1 THEN profit-((profit * .01) * wc_percent) ELSE profit END END profit';
