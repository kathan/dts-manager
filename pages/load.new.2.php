...
class load_table extends dts_table
{
	...
	function get_carrier_search_result_module()
	{
		...
		$t = new Template();
		$t->register_modifier("strtotime", "strtotime");
		
		$t->assign('load_id', $this->load_id);
		$t->assign('carriers', DB::to_array($re));
		$c .= $t->fetch(App::$temp.'load_carrier_search_module.tpl');
		return $c;
	}
	
	function get_carrier_search_result_module_old()
	{
		require_once("includes/portal.php");
		$c ='';
		$c .= $this->get_carrier_search_edit_module();
