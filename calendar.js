...
Calendar.prototype =
{
	show_calendar: function(callback, month, year)
	{
		...
		if(this.cal_tab)
		{
			document.body.removeChild(this.cal_tab);
			//document.body.removeChild(document.getElementById('cal_tab'));
		}
		this.cal_tab = document.createElement('table');
		this.appendChild(this.cal_tab);
		//document.body.appendChild(this.cal_tab);
		this.cal_tab.id = 'cal_tab';
	
		var cal_head = document.createElement('thead');
		this.cal_tab.appendChild(cal_head);
		...
		cal_prev.innerHTML ='&lt;&lt;';
		cal_prev.onclick = 'function() { '+this.offsetParent.id+'.show_calendar('+prev_month+', '+prev_year+'); }';
		...
		cal_next.innerHTML ='&gt;&gt;';
		cal_next.onclick = function() { this.show_calendar(next_month, next_year); };
		...
		for(r = 1; r <= weeks_in_month; r++)
		{
			...
			for(c = 1; c <= 7; c++)
			{
				...
				if (d_o_m > 0 && d_o_m <= days_in_month)
				{
					...
					callback_str = 'function() { '+callback+'('+d_o_m+', '+month+', '+year+'); }';
					cal_day_div.onclick = callback_str;
					d_o_m++;
				}
			}
		}	
	...
	{
		this.cal_tab.style.visibilty = 'hidden';
	}
}