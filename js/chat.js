...
function restructureChatBoxes() {
	...
	for (x in chatBoxes) {
		...
		if ($("#chatbox_"+chatboxtitle).css('display') != 'none') {
			if (align == 0) {
				$("#chatbox_"+chatboxtitle).css('right', 20+$('#online_users').width());
			} else {
				width = (align)*(225+7)+20+$('#online_users').width();
...
function createChatBox(chatboxtitle,minimizeChatBox) {
	...
	var n = chatBoxes.push(chatboxtitle);
	//alert(chatBoxes[n-1]);
	if (chatBoxeslength == 0) {
		$("#chatbox_"+chatboxtitle).css('right', 20+$('#online_users').width());
	} else {
		
		width = (chatBoxeslength)*(225+7)+20+$('#online_users').width();
		$("#chatbox_"+chatboxtitle).css('right', width+'px');
	}
