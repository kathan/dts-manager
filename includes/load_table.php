...
class load_table extends dts_table
{
	...
	function load_table()
	{
		...
		$this->add_table_params('page', 'load');
		...
		$this->tab_menu->add_tab("http://".HTTP_ROOT."/?page=load&action=$this->search_edit", $this->search);
		$this->tab_menu->add_tab("http://".HTTP_ROOT."/?page=load", 'Boards');
		//$this->tab_menu->add_tab("http://".HTTP_ROOT."/?page=load&action=$this->new_str", 'Open');
		$this->tab_menu->add_tab("http://".HTTP_ROOT."/?page=load&action=$this->all", 'List');
	...
	function render()
	{
		...
		if(logged_in())//1
		{
			...
			//include_once("includes/portal_style2.php");
			if(isset($_REQUEST[$this->portal]))
			{
				...
				{//3
					...
					case $this->repeat:
						$this->repeat_load();
						header("location: http://".HTTP_ROOT."/?page=load&action=$this->view_str&load_id=$this->load_id");
						break;
					case $this->new_str:
						$this->create_new();
						if($this->load_id)
	...
	function get_carriers()
	{
		...
		$sql = "SELECT	c.carrier_id, phys_address,
						...
						CONCAT('<center><input type=\"button\" value=\"Rate Conf\" onclick=\"javascript:open_rate_conf(\'http://".HTTP_ROOT."/?page=load&portal=$this->rate_conf_str&load_id=$this->load_id&carrier_id=',c.carrier_id,'&".SMALL_VIEW."\', \'load_carrier_".$this->load_id."_',c.carrier_id,'\')\">') rate_conf_button ";
	...
	/*function get_carriers_old()
	{
		//Needs work
		require_once"includes/portal.php";
		$sql = "SELECT c.carrier_id, CONCAT('<a href=\"#\" onclick=\"javascript:popUp(\'http://".HTTP_ROOT."/?page=load_carrier&action=$this->edit_str&load_id=$this->load_id&carrier_id=',c.carrier_id,'&".SMALL_VIEW."\', \'load_carrier_".$this->load_id."_',c.carrier_id,'\')\">',name,'</a>') name ";
		
		$sql .= ", CONCAT(main_phone_number, '<br>', fax) phone_fax,  booked_salesperson, notes ";
		if(logged_in_as('admin'))
		{
			$sql .= " ,CONCAT(	'<a href=\"#\" onclick=\"delete_carrier(',
							c.carrier_id,
		...
		$sql .= "		FROM load_carrier lc, carrier c
						WHERE load_id = $this->load_id
						AND c.carrier_id = lc.carrier_id";
		$c = $this->style("
		#load_carrier_portal .tableContainer{height:99% !important;}
		#load_carrier_portal table>tbody{height:10em !important;}");
		$p = new portal($sql);
		$p->set_table($this->load_carrier);
	...
	function get_warehouses()
	{
		...
		//$open_time = "CONCAT('<INPUT TYPE=\"text\" id=\"action=update&table=load_warehouse&warehouse_id=',w.warehouse_id,'&load_id=', load_id, '&open_time=\" name=\"open_time\" value=\"',TIME_FORMAT(open_time, '%l:%i %p'),'\" SIZE=8  onchange=\"db_save(this.id, this.value);column_updated(this);update_warehouse_portal();\">') open_time,";
		//$open_time = "CONCAT('<INPUT TYPE=\"text\" id=\"action=update&table=load_warehouse&warehouse_id=',w.warehouse_id,'&load_id=', load_id, '&open_time=\" name=\"open_time\" value=\"',TIME_FORMAT(open_time, '%l:%i %p'),'\" SIZE=8  onchange=\"db_save(this);column_updated(this);update_warehouse_portal();\">') open_time,";
		//$sql .= $open_time;
		...
		//$act_col = " act_date,";
				//echo "<text>$act_col</text>";
		$sql .= $act_col;
		//echo "<code>$act_col</code>";
		/*$activity_time = "CONCAT('<INPUT TYPE=\"text\" id=\"action=update&table=load_warehouse&warehouse_id=',w.warehouse_id,'&load_id=', load_id, '&activity_time=\" name=\"activity_time\" value=\"',TIME_FORMAT(activity_time, '%l:%i %p'),'\" SIZE=8 onchange=\"db_save(this.id, this.value);column_updated(this);update_warehouse_portal();\">') activity_time,";
		$sql .= $activity_time;*/
		
		//$close_time = "CONCAT('<INPUT TYPE=\"text\" id=\"action=update&table=load_warehouse&warehouse_id=',w.warehouse_id,'&load_id=', load_id, '&close_time=\" name=\"close_time\" value=\"',TIME_FORMAT(close_time, '%l:%i %p'),'\" SIZE=8 onchange=\"db_save(this.id, this.value);column_updated(this);update_warehouse_portal();\">') close_time,";
		//$close_time = "CONCAT('<INPUT TYPE=\"text\" id=\"action=update&table=load_warehouse&warehouse_id=',w.warehouse_id,'&load_id=', load_id, '&close_time=\" name=\"close_time\" value=\"',TIME_FORMAT(close_time, '%l:%i %p'),'\" SIZE=8 onchange=\"db_save(this);column_updated(this);update_warehouse_portal();\">') close_time,";
		//$sql .= $close_time;
		//$complete = "CONCAT(	'<INPUT TYPE=\"checkbox\" id=\"action=update&table=load_warehouse&warehouse_id=', w.warehouse_id, '&load_id=', load_id, '&complete=\" name=\"complete\" onchange=\"db_save(this.id, this.value);column_updated(this);update_warehouse_portal();\"', IF(lw.complete = 1, 'checked', 'unchecked'), '>') complete,";
	...
	function get_money_edit_module()
	{
		...
		//$c .= "<td>GP</td><td><div id='gp'></div></td>";
		//$c .= "<td>GPP</td><td><div id='gpp'>%</div></td></tr>";
		$c .= "</tr></table>";
		$c .= "</td><td>";
		$c .= "<table width='100%' border=0><tr>
	...
	function get_warehouse_search_edit_module()
	{
		...
		//$hi = new hidden_input('customer_id', $_REQUEST['customer_id']);
		//$wt->add_input($hi);
		
		$hi1 = new hidden_input(SMALL_VIEW);
		$wt->add_input($hi1);
		
		$hi2 = new hidden_input('page', 'load');
		$wt->add_input($hi2);
		...
		$o =& $col->get_edit_html();
		$o->set_size('5');
		//$test =& $wt->get_column('warehouse_id');
		//$test2 =& $test->get_edit_html();
		echo $test2->size;
		
		$wt->insert_column('name');
		$wt->insert_column('city');
		$wt->insert_column('state');
		//$cust = $this->get_customer($_REQUEST['customer_id']);
	...
	function get_carrier_search_result_module()
	{
		...
		$sql = "SELECT	CONCAT('S', carrier_id) carrier_id,
						...
						phys_city,
						phys_state,
						CONCAT('<input type=\"button\" value=\"Add\" onclick=\"add_load_carrier(',carrier_id,',\'',insurance_expires,'\')\">') `add`
																																				
				FROM carrier ";
				//CONCAT('<input type=\"button\" name=\"Add\" value=\"Add\" onclick=\"add_load_carrier(',carrier_id,',\'',insurance_expires,'\')\">') `add`
		$clause = 'WHERE';
		$where='';
	...
	function get_load_edit_2()
	{
		...
		$auth_edit=false;
		if(logged_in_as('admin') || get_user_id() == $r['order_by'])
		{
			$auth_edit=true;
		}
		$c .= $this->popup_script();
		...
		//==== Repeat Load Button ====
		$c .= "<form method='post' action='http://".HTTP_ROOT."/?page=load&action=$this->repeat&load_id=$this->load_id'><input type='submit' value='Repeat'></form>";
		//$c .= "<a href='http://".HTTP_ROOT."/?page=load&load_id=$this->load_id&portal=$this->confirm&".SMALL_VIEW."'>Rate Confirmation</a>";
						
		$c .= "<table width='100%'><tr>";
		$c .= "<td valign='top' width='33%' class='bottom_pad'>";
		//==== Order ====
		
		$c .= "<fieldset style='height:6em'><legend>Order</legend>";
		
		$c .= "<div id='order_module'>";
		$c .= "<table width='100%' border=0>";
		$c .= '<tr><td>Order By:</td><td>';
		//if(logged_in_as('admin'))
		//{
		//	$c .= $this->fetch_edit('order_by', $r['order_by']);
		//}else{
			$c .= $this->get_order_by_name();
		//}
		$c .= "</td></tr>";
		$c .= "<tr><td>Ordered Date:</td>";
		$c .= "<td>".MySQL_Date_To_format($r['order_date'], $this->php_date_format)."</td></tr>";
		$c .= "<tr><td>Activity Date</td><td>".$this->fetch_edit('activity_date', $r['activity_date'])."</td>";
		...
		$c .= "
		<div class='outer'>
			<div id='rating_module' class='inner' style='height:$this->row_height'>
				<p class='tab_label'>Rating</p>";
		$c .= "<table width='100%' border=0>";
		
		$c .= '<tr><td class="right">Rating Code:</td><td class="left">'.$this->fetch_edit('rating', $r['rating']).'</td><tr>';
		$c .= '<tr><td class="right">'.$this->fetch_edit('cancelled', $r['cancelled']).'</td><td class="left">Cancel</td></tr>';
		$c .= "</table>";
		...
		$c .= "
		<div class='outer'>
			<div class='inner' style='height:$this->row_height'>
				<p class='tab_label'>";
		if($auth_edit)
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&action=$this->edit_str&module=$this->problem_str&load_id=$this->load_id&".SMALL_VIEW."')\">Problem</a>";
		}else{
			$c .= 'Problem';
		}
		$c .= "</p>
		...
		if($auth_edit)
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&action=$this->customer_search_edit&load_id=$this->load_id&".SMALL_VIEW."', 700,550)\">Customer</a>";
		}else{
			$c .= 'Customer';
		}
		//$c .= $r['customer_id'];
		...
		if($auth_edit)
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&load_id=$this->load_id&action=$this->warehouse_search_edit&customer_id=$r[customer_id]&".SMALL_VIEW."', 700, 550)\">Warehouse</a>";
		}else{
			$c .= 'Warehouse';
		}
		$c .= "</p>";
		...
		if($auth_edit)
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&action=$this->edit_str&module=$this->money_str&load_id=$this->load_id&".SMALL_VIEW."',0,925,250)\">Money</a>";
		}else{
			$c .= 'Money';
		}
		$c .= "</p><div id='money_module'></div>";
		$c .= "</div></div>";
		$c .= $this->script("
		...
		if($auth_edit)
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&load_id=$this->load_id&action=$this->carrier_search_edit&".SMALL_VIEW."','carrier_search_'+$this->load_id, 700, 550)\">Carrier</a>";
		}else{
			$c .= 'Carrier';
		}
		$c .= "</p><div id='load_carrier_portal'></div>";
		$c .= "</div></div>";
		$c .= $this->script("
					function cancel()
					{
	...
	function get_load_edit()
	{
		...
		{
			$c .= "<a href='#' onclick=\"javascript:popUp('http://".HTTP_ROOT."/?page=load&action=$this->edit_str&module=$this->money_str&load_id=$this->load_id&".SMALL_VIEW."',0,1064,240)\">Money</a>";
		}else{
			$c .= 'Money';
		}
	...
	function get_new()
	{
		...
		$c .= "<input type='hidden' name='page' value='load'>";
	...
	function get_daily_profit($date)
	{
		/*$sql = "SELECT 
				sum(
					...
					(carrier_other * carrier_other_amount))) profit
	...
	/*function get_loads_new($date, $type)
	{
		require_once'includes/portal.php';
		$q = new portal();
		$q->set_sql("	SELECT 	l.load_id,
								IFNULL(
	...
	function get_loads($date, $type)
	{
		require_once'includes/portal.php';
		$q = new portal();
		//echo "$this->date_format $this->time_format";
