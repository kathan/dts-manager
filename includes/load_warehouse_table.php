...
class load_warehouse_table extends dts_table{
	...
	function load_warehouse_table(){
		parent::__construct('load_warehouse');
	...
	function get_load_carrier($load_id, $carrier_id){
		$sql ="	SELECT *
				FROM `load_carrier`
				WHERE load_id = ?
				AND carrier_id = ?";
		$binds = [$load_id, $carrier_id];
		$stmt = App::$db->prepare($sql);
		$result = $stmt->execute($binds);
		if($result){
			return $stmt->fetchAll(PDO::FETCH_ASSOC)[0];
	...
	function get_load_warehouse_edit(){
		...
		return $t->fetch(App::getTempDir().'load_warehouse_edit.tpl');
	}
	...
	function get_load_warehouse($load_id, $warehouse_id){
		$sql ="	SELECT	*
			...
			WHERE lw.warehouse_id = ?
			AND load_id = ?
			AND w.warehouse_id = lw.warehouse_id";
		$binds = [$warehouse_id, $load_id];
		$stmt = App::$db->prepare($sql);
		$result = $stmt->execute($binds);
		if($result){
